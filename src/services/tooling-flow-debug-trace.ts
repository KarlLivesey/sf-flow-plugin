...
export class ToolingFlowDebugTrace {
  ...
  public async renew(trace: TraceState, request: FlowDebugExecutionRequest): Promise<TraceState> {
    ...
    if (current === undefined || !traceMatches(current, trace.temporary)) {
      throw flowDebugCleanupFailed(
        `TraceFlag "${trace.traceFlagId}" changed before temporary tracing could be renewed; its current settings were not overwritten.`
      );
    }
    const change = {
      debugLevelId: trace.debugLevelId,
    ...
    if (!result.success) {
      return saveFailure(change.apiName, `renew temporary TraceFlag "${trace.traceFlagId}"`, result);
    }
    return { ...trace, temporary: temporarySnapshot(change) };
  }
