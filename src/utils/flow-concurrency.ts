 ...
 */
import { flowActiveVersionMismatch, flowLatestVersionMismatch } from '../errors/flow-errors.js';
...
export function assertExpectedLatestVersion(
  apiName: string,
  expected: FlowVersionNumber | undefined,
  actual: FlowVersionNumber | null
): void {
  if (expected !== undefined && expected !== actual) {
    throw flowLatestVersionMismatch(apiName, expected, actual);
  }
}
