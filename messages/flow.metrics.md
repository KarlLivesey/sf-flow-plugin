...
the selected metadata; apply policy thresholds in downstream CI or policy tooling. Maximum path depth and loop nesting
are explicitly reported as upper bounds: each strongly connected component is condensed and all of its elements are
counted, so the bound can exceed any non-repeating connector path through a cyclic component.
